...
while(t < GlobalConfig.maxMutateTime and (time.time()-duration) < 21600):
    controller.excute()
    try:
        tensorflow_elapsed,torch_elapsed,mindspore_elapsed,tensorflow_fps,torch_fps,mindspore_fps,complexity,diff_tf_torch_max,diff_torch_mindspore_max,diff_tf_mindspore_max,fitness = worker.excute()
        print("第" + str(t) + "轮已经完成")
